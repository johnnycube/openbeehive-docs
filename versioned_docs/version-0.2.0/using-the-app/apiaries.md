...
The apiary form includes a map with a draggable pin. Type an address and the pin moves to match; drag the pin (or tap the map) and the address field is filled in from the position — falling back to plain coordinates when no address is known for that spot.

A status line under the map tells you what the lookup is doing:

- **Searching…** — the address lookup is running.
- **Location set** — the position was found and written into the form.
- **No match** — the address service knows no such place. Drop the pin on the map instead.
- **Search unavailable** — the lookup service cannot be reached (you may be offline). The pin and manual coordinates keep working; only the automatic address lookup is paused.
