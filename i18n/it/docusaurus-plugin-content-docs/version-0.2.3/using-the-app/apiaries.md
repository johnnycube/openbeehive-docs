...
Il modulo dell'apiario include una mappa con un segnaposto trascinabile. Digita un indirizzo e il segnaposto si sposta di conseguenza; trascina il segnaposto (o tocca la mappa) e il campo dell'indirizzo viene compilato dalla posizione — se per quel punto non è noto alcun indirizzo, vengono usate le coordinate.

Una riga di stato sotto la mappa mostra cosa sta facendo la ricerca:

- **Ricerca in corso…** — la ricerca dell'indirizzo è attiva.
- **Posizione impostata** — la posizione è stata trovata e inserita nel modulo.
- **Nessuna corrispondenza** — il servizio non conosce quel luogo. Posiziona il segnaposto sulla mappa.
- **Ricerca non disponibile** — il servizio non è raggiungibile (forse sei offline). Il segnaposto e le coordinate manuali continuano a funzionare; è in pausa solo la ricerca automatica dell'indirizzo.
