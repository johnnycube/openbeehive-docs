...
L'autenticazione con password richiede un **amministratore dell'istanza** dedicato, configurato
nell'ambiente anziché creato tramite la registrazione:

```bash
BEEHIVE_ADMIN_EMAIL=tu@example.com
BEEHIVE_ADMIN_PASSWORD=almeno-otto-caratteri
```

Il server garantisce questo account a ogni avvio: viene creato se manca, il suo ruolo viene
forzato ad amministratore e la sua password viene reimpostata al valore configurato.
Quest'ultimo punto funge anche da recupero della password per l'amministratore: cambia la
variabile e riavvia. La registrazione non concede mai il ruolo di amministratore, e l'email
dell'amministratore non deve essere quella dell'account demo.

### Istanze solo su invito

Se non vuoi che estranei creino account, imposta `BEEHIVE_REGISTRATION=false` per
chiudere la registrazione aperta. L'amministratore dell'istanza viene dall'ambiente, quindi
un'istanza nuova ne ha sempre uno. Tutti gli altri possono unirsi solo tramite link di
invito, che l'amministratore emette dalle Impostazioni e che devono corrispondere
all'indirizzo email invitato. La schermata di accesso mostra un avviso che l'istanza è solo
su invito.

Ogni nuovo account inizia con il proprio [tenant](/using-the-app/accounts-tenants)
personale; solo l'account amministratore configurato porta il ruolo di amministratore
dell'istanza.
...
e password. L'amministratore dell'istanza resta l'account indicato in
`BEEHIVE_ADMIN_EMAIL`, in qualunque modo acceda.
:::
