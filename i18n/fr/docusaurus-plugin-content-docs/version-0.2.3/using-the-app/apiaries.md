...
Le formulaire du rucher comprend une carte avec un repère déplaçable. Saisissez une adresse et le repère s'y déplace ; faites glisser le repère (ou touchez la carte) et le champ d'adresse se remplit à partir de la position — si aucune adresse n'est connue pour cet endroit, les coordonnées sont utilisées à la place.

Une ligne d'état sous la carte indique ce que fait la recherche :

- **Recherche…** — la recherche d'adresse est en cours.
- **Position définie** — la position a été trouvée et reportée dans le formulaire.
- **Aucun résultat** — le service d'adresses ne connaît pas ce lieu. Placez plutôt le repère sur la carte.
- **Recherche indisponible** — le service est injoignable (vous êtes peut-être hors ligne). Le repère et les coordonnées manuelles continuent de fonctionner ; seule la recherche automatique d'adresse est en pause.
