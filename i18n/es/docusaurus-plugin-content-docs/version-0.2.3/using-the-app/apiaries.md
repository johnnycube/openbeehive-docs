...
El formulario del colmenar incluye un mapa con un marcador arrastrable. Escribe una dirección y el marcador se mueve hasta ella; arrastra el marcador (o toca el mapa) y el campo de dirección se rellena a partir de la posición — si no se conoce ninguna dirección para ese punto, se usan las coordenadas.

Una línea de estado bajo el mapa indica qué está haciendo la búsqueda:

- **Buscando…** — la búsqueda de la dirección está en curso.
- **Ubicación establecida** — la posición se encontró y se escribió en el formulario.
- **Sin coincidencias** — el servicio de direcciones no conoce ese lugar. Coloca el marcador en el mapa.
- **Búsqueda no disponible** — no se puede contactar con el servicio (quizá estés sin conexión). El marcador y las coordenadas manuales siguen funcionando; solo se pausa la búsqueda automática de direcciones.
