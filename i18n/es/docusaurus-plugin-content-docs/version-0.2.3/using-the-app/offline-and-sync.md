...
Iniciar sesión en un dispositivo nuevo, o volver a abrir la aplicación después de borrar su
almacenamiento, empieza con una base de datos local vacía que se va llenando en segundo
plano. La aplicación hace visible ese estado en lugar de fingir que no hay nada que mostrar:

- Las listas muestran **marcadores de posición con brillo** mientras leen de la base de
  datos local.
- Mientras la primera descarga sigue en marcha, el panel y las listas de colmenares,
  colmenas y tareas muestran **«Sincronizando tus datos…»** en lugar de un estado vacío, de
  modo que nunca se te invita a «crear tu primer colmenar» sobre registros que están a
  punto de llegar.
- Los conjuntos de datos grandes aparecen **de forma progresiva**: cada lote recibido se
  muestra al instante, así que una primera sincronización larga va llenando la pantalla poco a
  poco en lugar de al final.

Solo cuando la aplicación sabe que los datos están completos muestra un estado vacío real. Si
el dispositivo está sin conexión o no se puede llegar al servidor, el aviso da paso a lo que
haya almacenado localmente, para que nada quede oculto tras un indicador de carga.
...
El guardado ocurre en tu dispositivo, así que prácticamente nunca falla. Si alguna vez ocurre — por ejemplo porque el almacenamiento del navegador está lleno o dañado — la aplicación lo dice claramente: el formulario permanece abierto con todo lo que escribiste y un mensaje de error explica qué salió mal. Nada desaparece en silencio.

En la **cuenta de demostración** pública el servidor no acepta subidas (la demo se reinicia con regularidad). Aun así puedes probarlo todo: tus cambios se guardan en tu dispositivo y simplemente se quedan ahí en lugar de sincronizarse.

Hay una situación en la que guardar funciona pero no perdura: si el navegador no puede
darle a la aplicación su almacenamiento privado, la aplicación recurre a una base de datos en
memoria y muestra el aviso **«El almacenamiento no está disponible: los cambios no se
conservarán en este dispositivo.»** Todo sigue funcionando durante la sesión, y los cambios
se siguen sincronizando con el servidor si has iniciado sesión, pero la copia local desaparece
al cerrar la pestaña. Esto ocurre en ventanas de navegación privada y cuando una segunda
pestaña de la aplicación sigue reteniendo el almacenamiento; consulta
[Solución de problemas](/knowledge-base/troubleshooting#storage-is-unavailable) para saber qué
hacer.
