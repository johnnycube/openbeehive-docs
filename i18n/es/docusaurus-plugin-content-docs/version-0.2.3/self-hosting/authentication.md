...
La autenticacion por contrasena necesita un **administrador de la instancia** dedicado, configurado en el entorno en lugar de creado mediante el registro:

```bash
BEEHIVE_ADMIN_EMAIL=tu@example.com
BEEHIVE_ADMIN_PASSWORD=al-menos-ocho-caracteres
```

El servidor garantiza esta cuenta en cada arranque: se crea si falta, su rol se fuerza a administrador y su contrasena se restablece al valor configurado. Esto ultimo sirve tambien como recuperacion de contrasena para el administrador: cambia la variable y reinicia. El registro nunca concede el rol de administrador, y el correo del administrador no debe ser el de la cuenta de demostracion.

### Instancias solo por invitacion

Si no quieres que desconocidos creen cuentas, establece `BEEHIVE_REGISTRATION=false` para cerrar el registro abierto. El administrador de la instancia viene del entorno, asi que una instancia nueva siempre tiene uno. Todos los demas solo pueden unirse mediante enlaces de invitacion, que el administrador emite desde los Ajustes y que deben coincidir con el correo invitado. La pantalla de inicio de sesion muestra un aviso de que la instancia es solo por invitacion.

Cada cuenta nueva comienza con su propio [tenant](/using-the-app/accounts-tenants) personal; solo la cuenta de administrador configurada lleva el rol de administrador de la instancia.
