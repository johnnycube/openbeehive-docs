...
Das Bienenstand-Formular enthält eine Karte mit einem verschiebbaren Pin. Gib eine Adresse ein und der Pin springt dorthin; ziehe den Pin (oder tippe auf die Karte) und das Adressfeld wird aus der Position gefüllt — gibt es für die Stelle keine Adresse, werden stattdessen die Koordinaten eingetragen.

Eine Statuszeile unter der Karte zeigt, was die Suche gerade tut:

- **Suche läuft…** — die Adresssuche arbeitet.
- **Standort gesetzt** — die Position wurde gefunden und ins Formular übernommen.
- **Keine Übereinstimmung** — der Adressdienst kennt diesen Ort nicht. Setze stattdessen den Pin auf der Karte.
- **Suche nicht verfügbar** — der Suchdienst ist nicht erreichbar (womöglich bist du offline). Pin und manuelle Koordinaten funktionieren weiter; nur die automatische Adresssuche pausiert.
